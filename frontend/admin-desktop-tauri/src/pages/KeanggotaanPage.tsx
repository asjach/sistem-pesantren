    ...
    <div className={PAGE_SHELL}>
      ...
      <ExcelTable
        ...
        addButton={canTambah || canUbah ? (
          <>
            {canUbah && (
              <Button id="btn_generate_nisk" disabled={busyId !== null} onClick={() => void generateSemua()}>
                {busyId !== null ? 'Memproses…' : 'Generate NISK'}
              </Button>
            )}
            {canTambah && (
              <Button id="btn_tambah_keanggotaan" onClick={() => setTambahOpen(true)}>
                Tambah
              </Button>
            )}
            {canTambah && canUbah && (
              <Button id="btn_buka_import_keanggotaan" onClick={() => setImportOpen(true)}>
                Import
              </Button>
            )}
          </>
