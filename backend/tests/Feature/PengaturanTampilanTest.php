<?php

namespace Tests\Feature;

use App\Models\Lembaga;
use App\Models\PengaturanTampilan;
use App\Models\PresetTabel;
use App\Models\User;
use Database\Seeders\RoleSeeder;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Support\Facades\DB;
use Tests\TestCase;

class PengaturanTampilanTest extends TestCase
{
    use RefreshDatabase;

    protected function setUp(): void
    {
        parent::setUp();
        $this->seed(RoleSeeder::class);
    }

    protected int $userSeq = 0;

    protected function makeUser(string $role, array $lembagaIds = []): User
    {
        $this->userSeq++;
        $u = User::create([
            'name' => ucfirst($role).' '.$this->userSeq,
            'email' => "tampilan{$this->userSeq}_".uniqid().'@example.com',
            'phone' => '08'.str_pad((string) (9000000000 + $this->userSeq * 137), 10, '0', STR_PAD_LEFT),
            'password' => 'password',
        ]);
        $u->assignRole($role);
        foreach ($lembagaIds as $lid) {
            DB::table('user_lembaga')->insert([
                'user_id' => $u->id, 'jenjang' => $lid,
                'created_at' => now(), 'updated_at' => now(),
            ]);
        }

        return $u;
    }

    /** @return array{0: Lembaga, 1: Lembaga} */
    protected function lembaga(): array
    {
        $mi = Lembaga::create(['nama' => 'Madrasah Ibtidaiyah', 'jenjang' => 'MI', 'is_active' => true]);
        $md = Lembaga::create(['nama' => 'Madrasah Diniyah', 'jenjang' => 'MD', 'is_active' => true]);

        return [$mi, $md];
    }

    public function test_admin_lembaga_hanya_bisa_mengatur_lembaganya(): void
    {
        [$mi, $md] = $this->lembaga();
        $adminMi = $this->makeUser('admin', [$mi->jenjang]);
        $adminMd = $this->makeUser('admin', [$md->jenjang]);

        // Sebar standar = super_admin saja: lintas lembaga ditolak.
        $this->actingAs($adminMi, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang, $md->jenjang],
            'data' => ['tema' => ['theme' => 'geist']],
        ])->assertStatus(403);

        // "semua" pun ditolak untuk admin lembaga.
        $this->actingAs($adminMi, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => 'semua',
            'data' => ['tema' => ['theme' => 'geist']],
        ])->assertStatus(403);

        // Lembaganya sendiri pun ditolak (hanya super_admin).
        $this->actingAs($adminMi, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => ['tema' => ['theme' => 'nusantara', 'mode' => 'gelap']],
        ])->assertStatus(403);

        // show tanpa param: otomatis lembaga tunggal (belum ada standar).
        $this->actingAs($adminMi, 'sanctum')->getJson('/api/admin/pengaturan-tampilan')
            ->assertStatus(200)
            ->assertJsonPath('data.jenjang', $mi->jenjang)
            ->assertJsonPath('data.versi', 0);

        // Lembaga lain belum diatur.
        $this->actingAs($adminMd, 'sanctum')->getJson('/api/admin/pengaturan-tampilan')
            ->assertJsonPath('data.versi', 0)
            ->assertJsonPath('data.tampilan', null);

        // Membaca lembaga pasangan (MI↔MD) diizinkan.
        $this->actingAs($adminMd, 'sanctum')->getJson("/api/admin/pengaturan-tampilan?jenjang={$mi->jenjang}")
            ->assertStatus(200);
    }

    public function test_admin_pesantren_ditolak_sebar_standar(): void
    {
        [$mi] = $this->lembaga();
        $adminFull = $this->makeUser('admin');

        // Admin pesantren (tanpa pivot, akses semua lembaga) tetap bukan super_admin.
        $this->actingAs($adminFull, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => ['tema' => ['theme' => 'geist']],
        ])->assertStatus(403);

        $this->assertSame(0, PengaturanTampilan::count());
    }

    public function test_super_admin_sebar_ke_semua_dan_versi_naik(): void
    {
        [$mi, $md] = $this->lembaga();
        $pusat = $this->makeUser('super_admin');

        $res = $this->actingAs($pusat, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => 'semua',
            'data' => [
                'tema' => ['density' => 'nyaman'],
                'grid' => ['rowH' => 28, 'align' => ['nama' => 'center']],
            ],
        ]);
        $res->assertStatus(201);
        $this->assertCount(2, $res->json('data'));
        $this->assertSame(2, PengaturanTampilan::count());

        $this->actingAs($pusat, 'sanctum')->getJson("/api/admin/pengaturan-tampilan/versi?jenjang={$mi->jenjang}")
            ->assertJsonPath('data.versi', 1);

        // Simpan ulang → versi naik, bukan baris baru.
        $this->actingAs($pusat, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => ['grid' => ['rowH' => 30]],
        ])->assertStatus(201);

        $this->assertSame(2, PengaturanTampilan::count());
        $this->actingAs($pusat, 'sanctum')->getJson("/api/admin/pengaturan-tampilan/versi?jenjang={$mi->jenjang}")
            ->assertJsonPath('data.versi', 2);
        $this->assertSame(30, PengaturanTampilan::where('jenjang', $mi->jenjang)->firstOrFail()->data['grid']['rowH']);

        // Super admin dapat melihat lembaga mana pun.
        $this->actingAs($pusat, 'sanctum')->getJson("/api/admin/pengaturan-tampilan?jenjang={$md->jenjang}")
            ->assertStatus(200)
            ->assertJsonPath('data.tampilan.tema.density', 'nyaman')
            ->assertJsonPath('data.versi', 1);
    }

    public function test_preset_aktif_disarikan_ke_lembaga_target(): void
    {
        [$mi, $md] = $this->lembaga();
        $pusat = $this->makeUser('super_admin');

        PresetTabel::create([
            'jenjang' => $mi->jenjang, 'table_key' => 'kelas', 'nama' => 'Ringkas',
            'kolom' => ['nama', 'tingkat'],
        ]);

        $this->actingAs($pusat, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$md->jenjang],
            'sumber_jenjang' => $mi->jenjang,
            'data' => ['presetAktif' => ['kelas' => 'Ringkas']],
        ])->assertStatus(201);

        $salinan = PresetTabel::where('jenjang', $md->jenjang)->where('table_key', 'kelas')->where('nama', 'Ringkas')->first();
        $this->assertNotNull($salinan);
        $this->assertEquals(['nama', 'tingkat'], $salinan->kolom);

        // Lembaga tanpa sumber tidak memunculkan preset baru.
        $this->actingAs($pusat, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$md->jenjang],
            'data' => ['presetAktif' => ['kelas' => 'Tidak Ada']],
        ])->assertStatus(201);
        $this->assertSame(1, PresetTabel::where('jenjang', $md->jenjang)->count());
    }

    public function test_lebar_dan_beku_menerima_nilai_null_untuk_menghapus(): void
    {
        [$mi] = $this->lembaga();
        $pusat = $this->makeUser('super_admin');

        // Nilai lebar/beku per tabel.
        $this->actingAs($pusat, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => [
                'lebar' => ['kelas' => ['nama' => 160]],
                'beku' => ['kelas' => 2],
            ],
        ])->assertStatus(201);

        // null = hapus lebar/beku tabel itu (dipakai tombol Reset/AutoFit).
        $this->actingAs($pusat, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => [
                'lebar' => ['kelas' => null],
                'beku' => ['kelas' => null],
            ],
        ])->assertStatus(201);

        $data = PengaturanTampilan::where('jenjang', $mi->jenjang)->firstOrFail()->data;
        $this->assertSame(['kelas' => null], $data['lebar']);
        $this->assertSame(['kelas' => null], $data['beku']);
    }

    public function test_toolbar_menyimpan_visibilitas_kontrol_per_tabel(): void
    {
        [$mi] = $this->lembaga();
        $pusat = $this->makeUser('super_admin');

        $this->actingAs($pusat, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => [
                'toolbar' => ['kelas' => ['cari' => false, 'kolom' => true]],
            ],
        ])->assertStatus(201);

        $data = PengaturanTampilan::where('jenjang', $mi->jenjang)->firstOrFail()->data;
        $this->assertSame(['kelas' => ['cari' => false, 'kolom' => true]], $data['toolbar']);

        $this->actingAs($pusat, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => [
                'toolbar' => ['kelas' => ['cari' => 'ya']],
            ],
        ])->assertStatus(422)->assertJsonValidationErrors(['data.toolbar.kelas.cari']);
    }

    public function test_validasi_data_dan_reset_ke_bawaan(): void
    {
        [$mi] = $this->lembaga();
        $adminMi = $this->makeUser('admin', [$mi->jenjang]);
        $super = $this->makeUser('super_admin');

        $this->actingAs($super, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => ['grid' => ['rowH' => 5]],
        ])->assertStatus(422)->assertJsonValidationErrors(['data.grid.rowH']);

        $this->actingAs($super, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => ['tema' => ['mode' => 'senja']],
        ])->assertStatus(422)->assertJsonValidationErrors(['data.tema.mode']);

        $this->actingAs($super, 'sanctum')->putJson('/api/admin/pengaturan-tampilan', [
            'jenjangs' => [$mi->jenjang],
            'data' => ['tema' => ['theme' => 'geist']],
        ])->assertStatus(201);

        $this->actingAs($adminMi, 'sanctum')->deleteJson('/api/admin/pengaturan-tampilan')
            ->assertStatus(200);

        $this->assertSame(0, PengaturanTampilan::count());
        $this->actingAs($adminMi, 'sanctum')->getJson('/api/admin/pengaturan-tampilan')
            ->assertJsonPath('data.tampilan', null);
    }
}
