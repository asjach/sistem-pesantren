...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('lembaga', function (Blueprint $table) {
            // Kunci alami: jenjang, mis. 'MI', 'MD', 'MTS', 'MLN' (satu baris per jenjang).
            $table->string('jenjang', 20)->primary();
            $table->string('nama'); // PENYATUAN: bukan 'nama_lembaga'
            $table->string('nama_singkat')->nullable(); // EMIS: nama singkatan
            $table->string('mudir_am')->nullable(); // kepala lembaga (kepala madrasah)
